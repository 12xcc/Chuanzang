...
const periodData = {
  ...
  2: { // 最近一周
    xAxisData: Array.from({ length: 7 }, (_, i) => dayjs().subtract(6 - i, 'day').format('YYYY-MM-DD')),
    data: {
  ...
  3: { // 最近一个月
    xAxisData: Array.from({ length: 6 }, (_, i) => dayjs().subtract(5 * i, 'day').format('YYYY-MM-DD')),
    data: {
  ...
  4: { // 最近一年
    xAxisData: Array.from({ length: 12 }, (_, i) => dayjs().subtract(11 - i, 'month').format('YYYY-MM')),
    data: {
...
const initChart = () => {
  ...
  const updateChart = () => {
    ...
    const timePeriodData = periodData[props.TimePeriod] || { xAxisData: [], data: allData };
    ...
    const filteredSeries = Object.entries(timePeriodData.data)
      ...
      .map(([name, data], index) => ({
        ...
        smooth: true,
        itemStyle: {
          color: ['#285AC8', '#6b5b95', '#88b04b', '#f7cac9', '#92a8d1', '#ffcc5c', '#ff6f61', '#6b5b95', '#88b04b', '#f7cac9', '#92a8d1', '#ffcc5c'][index % 12]
        }
      }));
  ...
  watch(() => [props.DiseaseType, props.TimePeriod], updateChart, { immediate: true });
};
